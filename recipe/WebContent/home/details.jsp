...
document.addEventListener('DOMContentLoaded', function() {
	...
	function updateLikeButton(likeStatus) {
	    ...
	    if (likeStatus === 'liked') {
	        ...
	        heartIcon.classList.remove('white-heart');
	    } else {
	        heartIcon.classList.remove('red-heart'); // 빨간색 하트 클래스 제거
	        heartIcon.classList.add('white-heart');
	    }
	}
});
				...
				<form class="d-flex">
	                    <c:choose>
							<c:when test="${empty sessionScope.customInfo.userId }">
		                        <button class="btn btn-outline-dark" type="button"  id="loginButton"> <!-- onclick="redirectToLogin()" 수정 -->
